...
import Propellor
import Propellor.Base (catchMaybeIO, hPutStr, liftIO, processTranscript, stderr, (</>), withPrivData)
...
import Propellor.Types.MetaTypes (MetaType(WithInfo), MetaTypes)
...
htpasswdPath :: String -> String
htpasswdPath site = htmlDir site </> ".htpasswsd"
  ...
  withPrivData (PrivFile passwdFile) anyContext $ \getHtpasswd ->
    property "Configure .htpasswd" $
      getHtpasswd $ \(PrivData htpasswdContent) -> do
        liftPropellor $ File.writeFileContent File.ProtectedWrite (htpasswdPath site) (lines htpasswdContent)
        pure MadeChange
